...
public class BeanUtils {
	...
	public static final String UNDERLYING_APACHE = "org.apache.commons.beanutils.BeanUtils";

	public static final String UNDERLYING_SPRING = "org.springframework.beans.BeanUtils";

	static {
		// 针对apache-common实现，需对Date类型做null值的默认转换，否则会抛异常
	...
	public static <T> List<T> batchTransform(final Class<T> clazz, List<? extends Object> srcList) {
		return batchTransform(clazz, srcList, UNDERLYING_SPRING);
	}

	public static <T> List<T> batchTransform(final Class<T> clazz, List<? extends Object> srcList,
			String underlyingMethod) {
		...
		for (Object srcObject : srcList) {
			result.add(transfrom(clazz, srcObject, underlyingMethod));
		}
		return result;
	}
	...
	public static <T> T transfrom(Class<T> clazz, Object src) {
		return transfrom(clazz, src, UNDERLYING_SPRING);
	}

	public static <T> T transfrom(Class<T> clazz, Object src, String underlyingMethod) {
		if (src == null) {
			return null;
		}
		T instance = null;
		...
		if (UNDERLYING_APACHE.equals(underlyingMethod)) {
			try {
				org.apache.commons.beanutils.BeanUtils.copyProperties(instance, src);
